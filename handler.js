      ...
      case 'remove':
        if (chat.welcome) {
          ...
          for (let user of participants) {
            let pp = 'https://i.ibb.co/JRnn4Xh/e5b994680e23.jpg'
            try {
              ...
              text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Selamat datang, @user!').replace('@subject', this.getName(jid)).replace('@desc', groupMetadata.desc) :
                (chat.sBye || this.bye || conn.bye || 'Sampai jumpa yok baca surah alfatihah ketik .alfatihah')).replace('@user', '@' + user.split`@`[0])
       ...
       case 'tourl':
            ...
            owgi = await this.downloadMediaMessage(boij)
...
klik buton di bawah ini untuk mematikan pesan ini
`.trim(), '', 'DISABLE DELETE', '.enable delete', {
      quoted: m.message,
      contextInfo: {
    ...
    async onCall(json) {
      ...
      async onCall(json) {
        ...
        await this.sendMessage(from, '*WOAH ADA ORANG GUYS MALAH NELPON BOT GADA AKHLAK!!*', MessageType.extendedText)
        await this.blockUser(from, 'add')
    }
}
