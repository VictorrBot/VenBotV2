let handler = async (m, { conn, args, command }) => {
    ...
    if (command.endsWith('all') || command.endsWith('semua')) {
        for (let i = 0; i < chat.length; i++) { // For loops
            await m.reply('Byee👋, Bot akan keluar dari group bocil ff', chat[i].jid)
        ...
        await m.reply('Byee👋, Bot akan keluar dari group para wibu', args[0])
        await conn.groupLeave(args[0])
        await m.reply('Sukses!!')
    } else {
        if (!m.isGroup) return global.dfail('group', m, conn)
        await m.reply('Byee👋, Bot akan keluar dari group sampah', m.chat) // WKWKW pesannya sama semua. gk kreatif :v
